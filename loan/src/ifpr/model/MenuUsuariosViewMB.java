...
public class MenuUsuariosViewMB {
	...
	public MenuUsuariosViewMB() {
		...
		loginControllerMB = context.getApplication().evaluateExpressionGet(
	...
	private void adicionarItensAdm() {
		...
		adicionarItem("Tec. Administrativos", "/adm/tec_adm.ifpr",
				"#{breadCrumbController.navigate(2)}", "user16", true, 2);

		adicionarItem("Tec. Esportivos", "/tec_adm/tec_esp.ifpr",
				"#{breadCrumbController.navigate(3)}", "user16", true, 3);

		adicionarItem("Coordenadores", "/tec_adm/coordenadorPea.ifpr",
				"#{breadCrumbController.navigate(4)}", "user16", true, 4);

		adicionarItem("Secretários", "/tec_adm/secretario.ifpr",
				"#{breadCrumbController.navigate(5)}", "user16", true, 5);

		adicionarItem("Estudantes", "/tec_adm/estudante.ifpr",
				"#{breadCrumbController.navigate(10)}", "user16", true, 10);
		model.addElement(submenu);
		submenu = new DefaultSubMenu("Gerenciar - Competição");

		adicionarItem("Modalidades", "/adm/modalidade.ifpr",
				"#{breadCrumbController.navigate(8)}", "user16", true, 8);

		adicionarItem("Times", "/tec_adm/time.ifpr",
				"#{breadCrumbController.navigate(9)}", "user16", true, 9);
	...
	public void adicionarItem(String value, String url, String command,
			String icon, boolean inSubmenu, int numeroItem) {
	...
	public void setBreadCrumbController(
